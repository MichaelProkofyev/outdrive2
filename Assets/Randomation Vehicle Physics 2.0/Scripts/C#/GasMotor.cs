...
public class GasMotor : Motor
{
	...
	public override void FixedUpdate()
	{
		...
		if (ignition)
		{
			...
			//Set torque
			if (targetDrive.feedbackRPM * Mathf.Sign(vp.localVelocity.z) > targetDrive.rpm)
			{
				targetDrive.torque = 0;
			}
			else
			{
				targetDrive.torque = torqueCurve.Evaluate(Mathf.Abs(targetDrive.feedbackRPM) * 0.001f) * Mathf.Lerp(targetDrive.torque, power * Mathf.Abs(System.Math.Sign(actualInput)), (1 - inertia) * Time.timeScale) * (boosting ? 1 + boostPowerCurve.Evaluate(Mathf.Abs(vp.localVelocity.z)) : 1) * health;
			}
			...
			{
				float torqueFactor = F.PowNatural(1f / outputDrives.Length, driveDividePower);
				float tempRPM = 0;
	...
	public override void Update()
	{
		...
		if (snd && ignition)
		{
			...
			pitchFactor = (actualAccel != 0 || vp.groundedWheels == 0 ? 1 : 0.5f) * (shifting ? (pitchIncreaseBetweenShift ? Mathf.Sin((transmission.shiftTime / transmission.shiftDelay) * Mathf.PI) : Mathf.Min(transmission.shiftDelay, F.PowNatural(transmission.shiftTime, 2)) / transmission.shiftDelay) : 1) * airPitch;
